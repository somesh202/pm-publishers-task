...
[
    ...
    const filteredItems = menuItems.filter(item => {
        if ((item.permissionId && [3, 4].includes(item.permissionId)) || (item.items && item.items.some(child => [5, 8].includes(child.permissionId)))) {
            return true;
        }
        return false;
    });
    ...
    const sendRequest = async (item) => {
      try {
          const response = await axios.post('https://pmponline.co.in/sdetest/requests.php', item, {
              headers: {
  ...
  processItems();
    ...
    const handleRatingChange = async (newRating) => {
        setRating(newRating);
        try {
            const response = await axios.post('https://pmponline.co.in/sdetest/requests.php', {
                method: 'POST',
          ...
          <h2>Rating</h2>
            {[1, 2, 3, 4, 5].map((value) => (
                <span
                    ...
                    style={{ cursor: 'pointer', color: value <= rating ? 'gold' : 'gray', margin: 'auto' }}
                >
                    ★
                </span>
            ))}
